...
contract DeFiModule is AxelarExecutable {
    ...
    mapping(string => uint) public rewardForCid;
    ...
    function payRewards(string memory cid) internal {
        uint totalBalance = rewardForCid[cid];
    ...
    function addRewards(string calldata cid) external payable {
        rewardForCid[cid] += msg.value;
